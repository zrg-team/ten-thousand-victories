...
// Preserve Vite's HMR handshake through the loopback proxy. Returning ordinary HTML to an
// upgrade request made the installed client stop after its first screenshot with a console error.
server.on('upgrade',(req,socket,head)=>{
 const target=new URL(upstream);const remote=connect(Number(target.port)||80,target.hostname,()=>{
 ...
 for(const s of [socket,remote]){sockets.add(s);s.on('close',()=>sockets.delete(s));}
...
try{const url=new URL(upstream);url.host=`127.0.0.1:${server.address().port}`;url.searchParams.set('bench','1');url.searchParams.set('retainedpaths','1');
 const child=spawn(process.execPath,[client,'--url',url.href,'--headless',process.env.HEADLESS??'false','--click-selector','#skill-ready','--actions-file',`${out}/actions.json`,'--iterations','2','--pause-ms','500','--screenshot-dir',`${out}/client`],{stdio:'inherit'});process.exitCode=await new Promise(resolve=>child.on('exit',resolve));
}finally{for(const socket of sockets)socket.destroy();server.closeAllConnections();server.close();}
