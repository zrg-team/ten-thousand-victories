...
Usage: conquest-ui-v3.py [--revision 3|4] export <id> <generated.png> | pack
...
assert REVISION in (3, 4), 'Supported revisions: 3, 4'
...
def export(icon_id, source):
    assert icon_id in {j['id'] for j in JOBS}, icon_id
...
def review(parts):
    sheet = Image.new('RGB', (180 * len(JOBS), 264), '#f3ecd8')
    draw = ImageDraw.Draw(sheet)
    font = ImageFont.truetype('C:/Windows/Fonts/arial.ttf', 14)
    labels = {'supplies': 'Materials', 'spears': 'Brace spears', 'horse': 'Charge',
              'skirmish': 'Spread', 'tortoise': 'Raise shields', 'bows': 'Shoot'}
    for i, job in enumerate(JOBS):
        x, part = i * 180, parts[job['id']]
        draw.text((x + 15, 12), labels[job['id']], fill='#2a2118', font=font)
