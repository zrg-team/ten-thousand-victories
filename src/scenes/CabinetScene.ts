...
import { t } from '../i18n';
import { preferredAscentRules } from '../game/rulesetOptions';
...
export class CabinetScene extends Phaser.Scene {
  ...
  private renderCabinet(): void {
    ...
    // V2's scope labels (menu pages follow the version the next reign will play): the combine thresholds as they are — three copies to Lv2,
    // five more to Lv3 — and the hand's slots as the house has them, not "up to three".
    const heroBody = this.ui.label(X + 12, 0, hero ? (preferredAscentRules().scopeLabels ? t('beta.cabinet.heroBody') : t('cabinet.hero.body')) : t('cabinet.hero.noneBody'), 'caption',
    ...
    (['1', '2', '3'] as const).forEach((step, index) => {
      ...
      const text = this.ui.label(x + 6, y + 18, step === '3' && preferredAscentRules().scopeLabels ? t('beta.cabinet.step3') : t(`cabinet.steps.${step}` as Parameters<typeof t>[0]), 'caption',
    ...
    for (let first = 0; first < shown.length; first += cols) {
      ...
      if (held) {
        ...
        scroll.content.add(cardFaceOverlay(this, { x, y: cy, width: cellW, height: cellH }, {
          ...
          held: held.copies,
          ...(preferredAscentRules().scopeLabels ? { scoped: true, owned: true } : {}),
        }));
